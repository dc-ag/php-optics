...
class Lens
{
    ...
    public function update(object $in, mixed $replacement): object
    {
        ...
        if ($this->toType instanceof Type) {
            $toTypeName = $this->toType instanceof Type ? Type::gettypeEquivalent($this->toType) : $this->toType;
            $actualType = \is_object($replacement) ? \get_class($replacement) : \gettype($replacement);
            if ($toTypeName !== $actualType) {
                throw new \InvalidArgumentException("replacement must be of type [$toTypeName] - got [$actualType].");
            }
